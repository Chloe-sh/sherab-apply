...
**상태: ✅ 배포 완료 (2026-08-08 저녁, v5). 2026-08-09 라이브 검증됨 — 허니팟·동의값 없는 POST가 'legacy-page' 문구로 거부되는 것 확인.**

> `Code.gs` 원본은 이 저장소가 public이라 `.gitignore`로 제외되어 있습니다.
> **정본은 마스터 시트 → 확장 프로그램 → Apps Script 편집기 안에만 있습니다.**
> ⚠️ 로컬 `sherab-retreat\apps-script\Code.gs`는 패치 이전 구본입니다. **편집기에 통째로 붙여넣으면 패치가 유실되니, 수정은 편집기 안에서 부분 수정으로 하세요.** 아래 배포 순서·방법 문단은 최초 적용 당시 기록으로 남겨둡니다.
